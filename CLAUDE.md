...
notice.css          # Styles shared by those two notice pages (same fonts/tokens, follows lf_theme)
...
- `.github/workflows/checks.yml` validates JSON, internal links, the no‑inline‑script rule and `data-cfasync="false"` on every `<script>`.
- `smoke-test.yml` runs on push to `main`, daily, and on demand:
  - it waits for the deploy by hashing a changed static file; for an HTML-only push it waits until the page serves a line the push added (whole HTML can't be compared: the zone rewrites it);
  - then it checks the key routes, the CSP header, `/instagram` and `/pgp`, the 404, and `security.txt` expiry.
- `checks.yml` runs monthly too, so an expiring `security.txt` turns CI red. Renew it here, in proof and in status together (same `Expires`).
